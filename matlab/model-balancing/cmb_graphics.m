function [res, res_true, res_true_data] = cmb_graphics(prior, data, optimal, true, cmb_options, q_info, graphics_dir, kapp_max)
...
figure(1); 
res.c = scatter_plot(data.X.mean,optimal.X,[],met_color,'Metabolite levels [mM] (data)', 'Metabolite levels [mM] (fit)');

figure(2); 
%res = scatter_plot(log(data.E.mean),log(optimal.E),[],enz_color,'Enzyme levels [mM] (data)', 'Enzyme levels [mM] (fit)');
res.e = scatter_plot(data.lnE.mean,log(optimal.E),[],enz_color,'Enzyme levels [mM] (data)', 'Enzyme levels [mM] (fit)');

print([filenames.x], '-f1', '-depsc'); 
print([filenames.e], '-f2', '-depsc'); 

figure(3); 
res.KV = scatter_plot(qall_data(index.KV),qall_optimal(index.KV),qall_names(index.KV),fit_color,'K_{V} values [1/s] (data)', 'K_{V} values [1/s] (fit)');
...
res.KM = scatter_plot(qall_data(index.KM),qall_optimal(index.KM),qall_names(index.KM),km_color,'K_{M} values [mM] (data)', 'K_{M} values [mM] (fit)');
...
switch cmb_options.parameterisation,
  case 'Keq_KV_KM_KA_KI',
    ...
    figure(5); 
    res.Keq = scatter_plot(qall_data(index.Keq),qall_optimal(index.Keq),qall_names(index.Keq),keq_color,'K_{eq} values [unitless] (data)', 'K_{eq} values [unitless] (fit)');
      
    figure(6); 
    res.Kcatf = scatter_plot(qall_data(index.Kcatf),qall_optimal(index.Kcatf),qall_names(index.Kcatf),kcat_color,'k_{cat}^{+} values [1/s] (data)', 'k_{cat}^{+} values [1/s] (fit)');
    
    figure(7); 
    res.Kcatr = scatter_plot(qall_data(index.Kcatr),qall_optimal(index.Kcatr),qall_names(index.Kcatr),kcat_color,'k_{cat}^{-} values [1/s] (data)', 'k_{cat}^{-} values [1/s] (fit)');
      
    figure(8); 
    res.Kappf = scatter_plot(qall_data(index.Kcatf),log(kapp_max.forward),qall_names(index.Kcatf),kapp_color,'k_{cat}^{+} values [1/s] (data)', 'k_{app,max}^{+} values [1/s] (fit)');

    figure(9);
    %% plot only if some reverse kapp values have been determined
    if sum(isfinite(kapp_max.reverse)),
      res.Kappr = scatter_plot(qall_data(index.Kcatr),log(kapp_max.reverse),qall_names(index.Kcatr),kapp_color,'k_{cat}^{-} values [1/s] (data)', 'k_{app,max}^{-} values [1/s] (fit)');
    else 
...
if length(true),
  ...
  figure(11); 
  res_true.c = scatter_plot(true.X,optimal.X,[],met_color,'Metabolite levels [mM] (true)','Metabolite levels [mM] (fit)');
  
  figure(12); 
  res_true.e = scatter_plot(log(true.E),log(optimal.E),[],enz_color,'Enzyme levels [mM] (true)','Enzyme levels [mM] (fit)');

  figure(13); 
  res_true.KV = scatter_plot(qall_true(index.KV),qall_optimal(index.KV),qall_names(index.KV),fit_color,'K_{V} values [1/s] (true)', 'K_{V} values [1/s] (fit)');
  ...
  res_true.KM = scatter_plot(qall_true(index.KM),qall_optimal(index.KM),qall_names(index.KM),km_color,'K_{M} values [mM] (true)', 'K_{M} values [mM] (fit)');
  ...
  switch cmb_options.parameterisation,
    case 'Keq_KV_KM_KA_KI',
      ...
      figure(15); 
      res_true.Keq = scatter_plot(qall_true(index.Keq),qall_optimal(index.Keq),qall_names(index.Keq),keq_color,'K_{eq} values [unitless] (true)', 'K_{eq} values [unitless] (fit)');

      figure(16); 
      res_true.Kcatf = scatter_plot(qall_true(index.Kcatf),qall_optimal(index.Kcatf),qall_names(index.Kcatf),kcat_color,'k_{cat}^{+} values [1/s] (true)', 'k_{cat}^{+} values [1/s] (fit)');
      
      figure(17); 
      res_true.Kcatr = scatter_plot(qall_true(index.Kcatr),qall_optimal(index.Kcatr),qall_names(index.Kcatr),kcat_color,'k_{cat}^{-} values [1/s] (true)', 'k_{cat}^{-} values [1/s] (fit)');

      figure(18); 
      res_true.Kappf = scatter_plot(qall_true(index.Kcatf),log(kapp_max.forward),qall_names(index.Kcatf),kapp_color,'k_{cat}^{+} values [1/s] (true)', 'k_{app,max}^{+} values [1/s] (fit)');

      figure(19); 
      %% only if some reverse kapp values have been determined
      if sum(isfinite(kapp_max.reverse)),
        res_true.Kappr = scatter_plot(qall_true(index.Kcatr),log(kapp_max.reverse),qall_names(index.Kcatr),kapp_color,'k_{cat}^{-} values [1/s] (true)', 'k_{app,max}^{-} values [1/s] (fit)');
      else
        title('No reverse Kapp values determined');
      end
...
if length(true),
  ...
  figure(21); 
  res_true_data.c = scatter_plot(true.X,data.X.mean,[],met_color,'Metabolite levels [mM] (true)','Metabolite levels [mM] (data)');
  
  figure(22); 
  res_true_data.e = scatter_plot(log(true.E),log(data.E.mean),[],enz_color,'Enzyme levels [mM] (true)','Enzyme levels [mM] (data)');

  figure(23); 
  res_true_data.KV = scatter_plot(qall_true(index.KV),qall_data(index.KV),qall_names(index.KV),kin_color,'K_{V} values [1/s] (true)', 'K_{V} values [1/s] (data)');
  ...
  res_true_data.KM = scatter_plot(qall_true(index.KM),qall_data(index.KM),qall_names(index.KM),kin_color,'K_{M} values [mM] (true)', 'K_{M} values [mM] (data)');
  ...
  switch cmb_options.parameterisation,
    case 'Keq_KV_KM_KA_KI',
      ...
      figure(25); 
      res_true_data.Keq = scatter_plot(qall_true(index.Keq),qall_data(index.Keq),qall_names(index.Keq),keq_color,'K_{eq} values [unitless] (true)', 'K_{eq} values [unitless] (data)');

      figure(26); 
      res_true_data.Kcatf = scatter_plot(qall_true(index.Kcatf),qall_data(index.Kcatf),qall_names(index.Kcatf),kin_color,'k_{cat}^{+} values [1/s] (true)', 'k_{cat}^{+} values [1/s] (data)');
      
      figure(27); 
      res_true_data.Kcatr = scatter_plot(qall_true(index.Kcatr),qall_data(index.Kcatr),qall_names(index.Kcatr),kin_color,'k_{cat}^{-} values [1/s] (true)', 'k_{cat}^{-} values [1/s] (data)');
...
function out = scatter_plot(xvalues,yvalues,names,color,xtitle,ytitle,mark_indices)
