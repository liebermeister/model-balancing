...
def to_sbtab(
    ...
    k_eq_df = pd.DataFrame(Keq.m_as(""), columns=["Keq"])
    ...
    kcat_df = pd.DataFrame(
        np.vstack([kcatf.m_as("1/s"), kcatr.m_as("1/s")]).T, columns=["kcatf", "kcatr"]
    )
    ...
    Km_df = pd.DataFrame(Km.m_as("mM").T, columns=metabolite_names)
    ...
    Ka_df = pd.DataFrame(Ka.m_as("mM").T, columns=metabolite_names)
    ...
    Ki_df = pd.DataFrame(Ki.m_as("mM").T, columns=metabolite_names)
...
to_sbtab(mbc, metabolite_names, reaction_names, state_names).write(
    f"../res/{config_fname}_convex.tsv"
)
...
for a in [0.0, 0.001, 0.01, 0.1, 0.5, 1.0]:
    ...
    to_sbtab(mb, metabolite_names, reaction_names, state_names).write(
        f"../res/{config_fname}_alpha_{a:.1g}.tsv"
    )
